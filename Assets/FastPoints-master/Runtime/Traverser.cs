...
namespace FastPoints
{
    ...
    public class Traverser
    {
        ...
        private static double DistanceToCamera(TraversalParams p, OctreeGeometryNode node)
        {
            Vector3 nodeCenter = node.boundingBox.Center + p.geometry.offset;
            return (nodeCenter - p.camPosition).magnitude; 
        }
        ...
        static void TraversalThread(object obj)
        {
            ...
            void TraverseNode(OctreeGeometryNode n, SimplePriorityQueue<OctreeGeometryNode> nodeQueue, Queue<OctreeGeometryNode> geomQueue, Queue<OctreeGeometryNode> deleteQueue)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    double priority = CalculatePriority(c, size);
                    if (size > minNodeSize || c.level <= 2)
                    {
                        loadLine += c.name + " ";
                        nodeQueue.Enqueue(c, (float)priority);
                    }
                    else if (c.Created)
                    {
                        deleteLine += c.name + " ";
            ...
            double CalculatePriority(OctreeGeometryNode n, double size)
            {
                double distance = DistanceToCamera(p, n);
                return -size / distance; // Negative to sort by min priority
            }
            ...
            while (!p.stopSignal)
            {
                ...
                Queue<OctreeGeometryNode> reversedNodes = new Queue<OctreeGeometryNode>(nodesToRender.Reverse());

                p.renderer.SetQueues(reversedNodes, nodesToDelete);
            }
        }
